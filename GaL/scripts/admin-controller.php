...
if(isset($_POST['delete-user'])){
 ...
 if(isset($_POST['promote-user'])){
  ...
  if (isset($_POST['add-game'])) {
     ...
     if (!in_array(strtolower(pathinfo(basename($_FILES["gtest"]["name"]),
     PATHINFO_EXTENSION)) , array("json"))) { array_push($errors, "Game test has to be json"); }

     // if (empty($gameimage1)) { array_push($errors, "Game images are required"); }
     // if (empty($gametext1)) { array_push($errors, "Game text is required"); }
     $imgerr = 0;
     $txterr = 0;
     $imgtypeerr = 0;
     $txttypeerr = 0;
     ...
     for($i=0; $i<$num; $i++)
     {
       switch ($i)
       {
          case 0:
              ...
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage1"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext1"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 1:
              if ($_FILES['gimage2']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext2']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage2"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext2"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 2:
              if ($_FILES['gimage3']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext3']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage3"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext3"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 3:
              if ($_FILES['gimage4']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext4']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage4"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext4"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 4:
              if ($_FILES['gimage5']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext5']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage5"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext5"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 5:
              if ($_FILES['gimage6']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext6']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage6"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext6"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 6:
              if ($_FILES['gimage7']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext7']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage7"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext7"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 7:
              if ($_FILES['gimage8']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext8']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage8"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext8"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 8:
              if ($_FILES['gimage9']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext9']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage9"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext9"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
          case 9:
              if ($_FILES['gimage10']['size'] < 1) { $imgerr = 1; }
              if ($_FILES['gtext10']['size'] < 1) { $txterr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gimage10"]["name"]),
              PATHINFO_EXTENSION)) , array("jpg", "jpeg", "png"))) { $imgtypeerr = 1; }
              if (!in_array(strtolower(pathinfo(basename($_FILES["gtext10"]["name"]),
              PATHINFO_EXTENSION)) , array("txt"))) { $txttypeerr = 1; }
              break;
        }
     }
     ...
     if (count($errors) == 0) {
     	...
     	$query = "INSERT INTO games (name) VALUES('$gamename')";
      ...
      mkdir('resources/games/'."$gamepath".'/'.'text/', 0777, true);
        ...
        // UPLOAD FILES: images, texts, test
        ...
        echo '<script type="text/javascript"> alert("Game added") </script>';
      }
     }
   }
